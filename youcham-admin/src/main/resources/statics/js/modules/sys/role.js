...
//            { label: '所属部门', name: 'deptName', sortable: false, width: 75 },
            ...
            /*{ label: '操作',  width: 80,
            	formatter: function(value, options, row){
     				return '<a href="JavaScript:;" onclick="vm.getUser(\''+row.roleId+'\')">绑定用户</a>'
     			}
            }*/
        ],
        viewrecords: true,
...
//                menu_ztree.expandAll(true);
        ...
        getUser:function(roleId){
            $.get(baseURL + "sys/user/getUserList?roleId="+roleId, function(r){          
            	layer.open({
	                type: 2,
	                offset: '20px',
	                maxmin:true,
	                ...
	                shade: 0.2,
	                shadeClose: false,
	                content:baseURL+ "modules/sys/user3.html?userIds="+r.uids,
	                btn: ['确定','关闭'],	                
	                yes: function (index, layero) {
	                	// 获取弹出层选中的值
						var body = layer.getChildFrame('body',index);
						var	myid = body.find('#getallid').html();
						var userIds=myid.split(",");
						if (isBlank(userIds)) {
							alert("请至少选中一条数据");
						} else {// 选中，则关闭弹出层
							console.log(myid);
							$.ajax({
			                    type: "POST",
			                    url: baseURL + "sys/user/roleAndUser?roleId="+roleId,
			                    contentType: "application/json",
